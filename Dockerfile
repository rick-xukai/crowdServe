...
RUN \
  ...
  elif [ -f package-lock.json ]; then npm ci --network-timeout 100000; \
  elif [ -f pnpm-lock.yaml ]; then yarn global add pnpm && pnpm i --network-timeout 100000 --frozen-lockfile; \
